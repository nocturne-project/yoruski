...
export type YoruqCardDesign = typeof yoruqCardDesigns[number];

/**
 * 質問ステータスの種類
 */
export const yoruqQuestionStatuses = [
	'pending',
	'answered',
	'deleted',
] as const;

export type YoruqQuestionStatus = typeof yoruqQuestionStatuses[number];

/**
 * YoruQuestion 質問エンティティ
 * - 質問者から回答者への匿名質問
 * - 回答ステータス管理
 * - メッセージカードデザイン選択
 * - E2E暗号化対応
 */
@Entity('yoruq_question')
@Index(['recipientId', 'status'])
@Index(['senderId'])
@Index(['createdAt'])
export class YoruqQuestion {
	@PrimaryColumn(id())
	public id: string;
	...
	public cardDesign: YoruqCardDesign;
	...
	public status: YoruqQuestionStatus;
	...
	constructor(data: Partial<YoruqQuestion>) {
		if (data == null) return;
