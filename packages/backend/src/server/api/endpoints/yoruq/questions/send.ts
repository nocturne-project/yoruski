...
import type { YoruqQuestionsRepository, UsersRepository } from '@/models/_.js';
import { DI } from '@/di-symbols.js';
import { YoruQuestionService } from '@/core/YoruQuestionService.js';
import { GetterService } from '@/server/api/GetterService.js';
import { yoruqCardDesigns } from '@/models/YoruqQuestion.js';
import { ApiError } from '../../../error.js';

/**
 * yoruq/questions/send
 * 匿名質問を送信する
 */
export const meta = {
	tags: ['yoruq'],
	...
	errors: {
		...
		yoruquestionDisabled: {
			message: 'Target user has not enabled Noqestion.',
			code: 'YORUQUESTION_DISABLED',
...
export const paramDef = {
	...
	properties: {
		...
		cardDesign: { type: 'string', enum: yoruqCardDesigns, default: 'default' },
	},
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			try {
				const question = await this.yoruQuestionService.sendQuestion({
					senderId: me.id,
				...
				if (err instanceof Error) {
					switch (err.message) {
						case 'YORUQUESTION_DISABLED':
							throw new ApiError(meta.errors.yoruquestionDisabled);
