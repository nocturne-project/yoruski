...
import type { YoruqQuestionsRepository, YoruqReportedQuestionsRepository, AbuseUserReportsRepository } from '@/models/_.js';
import { DI } from '@/di-symbols.js';
import { IdService } from '@/core/IdService.js';
import { ApiError } from '../../../error.js';

/**
 * yoruq/questions/report
 * 質問を通報する
 * - Misskey標準のAbuseUserReportを作成
 * - YoruqReportedQuestion中間テーブルに登録
 * - YoruqQuestion.isReportedをtrueに更新
 */
export const meta = {
	tags: ['yoruq'],
...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			// 質問の存在確認
			const question = await this.yoruqQuestionsRepository.findOneBy({ id: ps.questionId });
			if (question == null) {
			...
			// YoruqReportedQuestionを作成
			await this.yoruqReportedQuestionsRepository.insert({
