...
import type { YoruqQuestionsRepository, NotesRepository } from '@/models/_.js';
import { DI } from '@/di-symbols.js';
import { YoruQuestionService } from '@/core/YoruQuestionService.js';
import { ApiError } from '@/server/api/error.js';

/**
 * yoruq/questions/answer
 * 質問に回答する（回答ノートIDを紐付け）
 */
export const meta = {
	tags: ['yoruq'],
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			if (ps.encryptedAnswer) {
				try {
					const updated = await this.yoruQuestionService.updateQuestionStatusWithEncryptedAnswer(
						ps.questionId,
						me.id,
						ps.encryptedAnswer,
					);
			...
			try {
				const updated = await this.yoruQuestionService.updateQuestionStatus(
					ps.questionId,
					me.id,
					'answered',
					ps.noteId,
