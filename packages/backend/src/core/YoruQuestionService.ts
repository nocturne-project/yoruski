...
import type {
	YoruqUserSettingsRepository,
	YoruqQuestionsRepository,
	YoruqMutedUsersRepository,
	YoruqReportedQuestionsRepository,
	UsersRepository,
	BlockingsRepository,
	MutingsRepository,
} from '@/models/_.js';
...
import type { YoruqQuestionStatus, YoruqCardDesign } from '@/models/YoruqQuestion.js';
...
/**
 * YoruQuestion - 匿名質問箱サービス
 *
 * ユーザー設定管理、質問送受信、回答機能を提供
 */
@Injectable()
export class YoruQuestionService {
	constructor(
		@Inject(DI.config)
		...
		@Inject(DI.yoruqReportedQuestionsRepository)
		private yoruqReportedQuestionsRepository: YoruqReportedQuestionsRepository,
	 ...
	 */
	@bindThis
	public async getOrCreateUserSetting(userId: MiUser['id']): Promise<YoruqUserSetting> {
		let setting = await this.yoruqUserSettingsRepository.findOneBy({ userId });

		if (setting == null) {
			setting = new YoruqUserSetting({
				userId,
				isEnabled: false,
			...
			await this.yoruqUserSettingsRepository.insert(setting);
		}
	...
	public async updateUserSetting(
		userId: MiUser['id'],
		params: Partial<Omit<YoruqUserSetting, 'userId' | 'user' | 'createdAt' | 'updatedAt'>>,
	): Promise<YoruqUserSetting> {
		// 設定が存在するか確認
		await this.getOrCreateUserSetting(userId);

		// 更新
		await this.yoruqUserSettingsRepository.update(
			{ userId },
			{
				...params,
				updatedAt: new Date(),
		...
		return await this.yoruqUserSettingsRepository.findOneByOrFail({ userId });
	}
	 ...
	 */
	@bindThis
	public async isYoruQuestionEnabled(userId: MiUser['id']): Promise<boolean> {
		const setting = await this.yoruqUserSettingsRepository.findOneBy({ userId });
	...
	public async sendQuestion(params: {
		...
		cardDesign?: YoruqCardDesign;
	}): Promise<YoruqQuestion> {
		...
		if (!recipientSetting.isEnabled) {
			throw new Error('YORUQUESTION_DISABLED');
		}
		...
		// 質問を作成
		const question = new YoruqQuestion({
			...
			status: 'pending' as YoruqQuestionStatus,
			isReported: false,
		...
		await this.yoruqQuestionsRepository.insert(question);
		...
		this.sendDmNotification(recipientId, question.text).catch(err => {
			console.error('[YoruQuestion] DM notification failed:', err);
		});
	...
	private async sendDmNotification(recipientId: MiUser['id'], questionText: string): Promise<void> {
		...
		// ボットアカウントが未設定の場合はスキップ
		if (!meta.yoruqBotAccountId) {
			return;
		}

		// ボットアカウントと回答者の情報を取得
		const [botAccount, recipient] = await Promise.all([
			this.usersRepository.findOneBy({ id: meta.yoruqBotAccountId }),
		...
		// DM本文を作成
		const dmText = `YoruQuestionで質問されたにゃん！\n質問： ${truncatedQuestion}\nhttps://${this.config.host}/@${recipient.username}/yoruq`;
		...
		try {
			...
			console.error('[YoruQuestion] Failed to send DM notification:', err);
		}
	}
	...
	public async getReceivedQuestions(
		userId: MiUser['id'],
		options?: {
			status?: YoruqQuestionStatus;
			limit?: number;
			sinceId?: string;
			untilId?: string;
		},
	): Promise<YoruqQuestion[]> {
		const query = this.yoruqQuestionsRepository.createQueryBuilder('question')
	...
	public async updateQuestionStatus(
		questionId: YoruqQuestion['id'],
		userId: MiUser['id'],
		status: YoruqQuestionStatus,
		answerNoteId?: string | null,
		answerText?: string | null,
	): Promise<YoruqQuestion> {
		const question = await this.yoruqQuestionsRepository.findOneBy({ id: questionId });
		...
		const updateData: Partial<YoruqQuestion> = {
			status,
		...
		await this.yoruqQuestionsRepository.update(
			{ id: questionId },
			updateData,
		);
		...
		if (status === 'answered' && answerNoteId && question.senderId) {
			this.sendAnswerDmNotification(question.senderId, answerNoteId).catch(err => {
				console.error('[YoruQuestion] Answer DM notification failed:', err);
			});
		}

		return await this.yoruqQuestionsRepository.findOneByOrFail({ id: questionId });
	}
	...
	private async sendAnswerDmNotification(senderId: MiUser['id'], noteId: string): Promise<void> {
		...
		try {
			...
			// DM送信に失敗してもエラーをログに記録するだけで処理を継続
			console.error('[YoruQuestion] Failed to send answer DM notification:', err);
		}
	}
	...
	public async updateQuestionStatusWithEncryptedAnswer(
		...
		await this.yoruqQuestionsRepository.update(
			{ id: questionId },
			{
				status: 'answered' as YoruqQuestionStatus,
				encryptedAnswer,
				answeredAt: new Date(),
			},
		);
		...
		if (question.senderId) {
			this.sendEncryptedAnswerDmNotification(question.senderId).catch(err => {
				console.error('[YoruQuestion] Encrypted answer DM notification failed:', err);
			});
		}
	...
	private async sendEncryptedAnswerDmNotification(senderId: MiUser['id']): Promise<void> {
		...
		// DM本文を作成（復号ツールリンク付き）
		const dmText = `暗号化された質問に回答があったにゃん！\n復号ツールで確認してね：\nhttps://${this.config.host}/yoruq/decryption`;
		...
		try {
			...
			// DM送信に失敗してもエラーをログに記録するだけで処理を継続
			console.error('[YoruQuestion] Failed to send encrypted answer DM notification:', err);
		}
	}
	...
	public async deleteQuestion(
		...
		await this.yoruqQuestionsRepository.update(
			{ id: questionId },
			{ status: 'deleted' as YoruqQuestionStatus },
		);
	}
	...
	public async muteUser(userId: MiUser['id'], mutedUserId: MiUser['id']): Promise<void> {
		...
		const existing = await this.yoruqMutedUsersRepository.findOneBy({ userId, mutedUserId });
		if (existing) {
			return; // 既にミュート済み
		}

		const mutedUser = new YoruqMutedUser({
			id: this.idService.gen(),
			userId,
			mutedUserId,
		});

		await this.yoruqMutedUsersRepository.insert(mutedUser);
	}
	...
	public async unmuteUser(userId: MiUser['id'], mutedUserId: MiUser['id']): Promise<void> {
		await this.yoruqMutedUsersRepository.delete({ userId, mutedUserId });
	}
	 ...
	 */
	@bindThis
	public async getMutedUsers(userId: MiUser['id']): Promise<YoruqMutedUser[]> {
		return await this.yoruqMutedUsersRepository.find({
			where: { userId },
	...
	private async isMuted(senderId: MiUser['id'], recipientId: MiUser['id']): Promise<boolean> {
		// 質問箱用ミュートをチェック
		const noqMuted = await this.yoruqMutedUsersRepository.findOneBy({
			userId: recipientId,
