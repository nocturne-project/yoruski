...
<MkStickyContainer>
	...
	<div class="_spacer" style="--MI_SPACER-w: 700px;">
		<div class="_gaps_m">
			<MkInfo>{{ i18n.ts._yoruq.e2e.description }}</MkInfo>
			<MkInfo warn>{{ i18n.ts._yoruq.e2e.securityWarning }}</MkInfo>

			<div class="decryption-form">
				<FormSection>
					<template #label>{{ i18n.ts._yoruq.e2e.decryptionPage }}</template>

					<div class="_gaps_m">
						<MkInput v-model="password" type="password">
							<template #label>{{ i18n.ts._yoruq.e2e.enterPassword }}</template>
							<template #caption>{{ i18n.ts._yoruq.e2e.passwordNote }}</template>
						</MkInput>

						<MkTextarea v-model="cipherText" :placeholder="String(i18n.ts._yoruq.e2e.decryptionPageDescription)">
							<template #label>{{ i18n.ts._yoruq.questionText }}</template>
						</MkTextarea>

						<MkButton :disabled="!canDecrypt || decrypting" primary @click="doDecrypt">
							<i class="ti ti-lock-open"></i> {{ i18n.ts._yoruq.e2e.decrypt }}
						</MkButton>

						<div v-if="decryptionError" class="error">
							<MkInfo warn>{{ i18n.ts._yoruq.e2e.decryptionFailed }}</MkInfo>
						</div>

						<div v-if="decryptedText !== null" class="result">
							<div class="result-label">{{ i18n.ts._yoruq.e2e.decryptedText }}</div>
...
import { decrypt } from '@/utility/yoruq-crypto.js';
...
definePage(() => ({
	title: i18n.ts._yoruq.e2e.decryptionPage,
