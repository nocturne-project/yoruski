...
async function load() {
	loading.value = true;
	try {
		mutedUsers.value = await misskeyApi('yoruq/mute/list', {}) as MutedUser[];
	} catch (err) {
		console.error(err);
	} finally {
		loading.value = false;
...
async function unmute(userId: string) {
	try {
		await misskeyApi('yoruq/mute/delete', { userId });
...
<template>
<div class="yoruq-muted-users">
	<div v-if="loading" class="loading">
		Loading...
	</div>

	<div v-else-if="mutedUsers.length === 0" class="no-users">
		{{ i18n.ts._yoruq.noMutedUsers }}
	</div>
