...
async function load() {
	loading.value = true;
	try {
		const settings = await misskeyApi('yoruq/settings/get', {});
...
async function save() {
	...
	saved.value = false;
	try {
		await misskeyApi('yoruq/settings/update', {
...
async function announce() {
	if (!isEnabled.value) {
		os.alert({
			type: 'warning',
			title: i18n.ts._yoruq.questionBoxDisabled,
			text: i18n.ts._yoruq.enableQuestionBox,
		});
		return;
	}
	...
	try {
		await misskeyApi('yoruq/announce', { visibility });
		os.toast(i18n.ts._yoruq.announceSent);
	} catch (err) {
		console.error(err);
		os.alert({
			type: 'error',
...
async function generateE2eKey() {
	if (e2ePassword.value.length < 4) {
		os.alert({
			type: 'warning',
			title: i18n.ts._yoruq.e2e.passwordTooShort,
			text: i18n.ts._yoruq.e2e.passwordMinLength,
		});
		return;
	}
	...
	if (e2ePublicKey.value) {
		const { canceled } = await os.confirm({
			type: 'warning',
			title: i18n.ts._yoruq.e2e.regenerateWarningTitle,
			text: i18n.ts._yoruq.e2e.regenerateWarningText,
		});
		if (canceled) return;
	}
	...
	try {
		const result = await misskeyApi('yoruq/settings/generate-key', {
		...
		os.toast(String(i18n.ts._yoruq.e2e.keyGenerated));
	} catch (err) {
		console.error(err);
		os.alert({
			type: 'error',
...
<template>
<div v-if="!loading" class="yoruq-settings">
	<MkSwitch v-model="isEnabled" class="setting-item">
		{{ i18n.ts._yoruq.enableQuestionBox }}
	</MkSwitch>

	<MkSwitch v-model="requireUsernameDisclosure" class="setting-item">
		{{ i18n.ts._yoruq.requireUsernameDisclosure }}
	</MkSwitch>

	<MkSwitch v-model="hideSensitiveQuestions" class="setting-item">
		{{ i18n.ts._yoruq.hideSensitiveQuestions }}
	</MkSwitch>

	<div class="setting-item">
		<label class="label">{{ i18n.ts._yoruq.notice }}</label>
		<MkTextarea
			v-model="notice"
			:placeholder="i18n.ts._yoruq.noticePlaceholder"
			:max="1000"
		/>
	</div>

	<div class="setting-item">
		<label class="label">{{ i18n.ts._yoruq.ngWordList }}</label>
		<p class="description">{{ i18n.ts._yoruq.ngWordListDescription }}</p>
	...
	<div v-if="isEnabled" class="announce-section">
		<label class="label">{{ i18n.ts._yoruq.announce }}</label>
		<p class="description">{{ i18n.ts._yoruq.announceDescription }}</p>
		<MkButton :disabled="announcing" @click="announce">
			<i class="ti ti-speakerphone"></i> {{ i18n.ts._yoruq.postAnnounce }}
		</MkButton>
	</div>

	<div v-if="isEnabled" class="e2e-section">
		<label class="label"><i class="ti ti-lock"></i> {{ i18n.ts._yoruq.e2e.title }}</label>
		<p class="description">{{ i18n.ts._yoruq.e2e.description }}</p>
		<MkInfo class="e2e-warning" warn>{{ i18n.ts._yoruq.e2e.securityWarning }}</MkInfo>

		<div v-if="e2ePublicKey" class="public-key-display">
			<label class="label">{{ i18n.ts._yoruq.e2e.yourPublicKey }}</label>
		...
		<div class="key-generation">
			<label class="label">{{ e2ePublicKey ? i18n.ts._yoruq.e2e.regenerateKey : i18n.ts._yoruq.e2e.generateKey }}</label>
			<p class="description">{{ i18n.ts._yoruq.e2e.passwordNote }}</p>
			<MkInput v-model="e2ePassword" type="password" :placeholder="i18n.ts._yoruq.e2e.passwordPlaceholder" class="password-input" />
			<MkButton :disabled="generatingKey || e2ePassword.length < 4" @click="generateE2eKey">
				<i class="ti ti-key"></i> {{ e2ePublicKey ? i18n.ts._yoruq.e2e.regenerate : i18n.ts._yoruq.e2e.generate }}
			</MkButton>
			<MkInfo v-if="keyGenerated" class="key-generated-message">{{ i18n.ts._yoruq.e2e.keyGenerated }}</MkInfo>
		</div>
	</div>
</div>
