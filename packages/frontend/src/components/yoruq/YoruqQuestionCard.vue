...
/**
 * YoruqQuestionCard.vue
 * 個別の質問カードコンポーネント
 * - YoruqMessageCardを使用したカードデザインプレビュー
 * - 送信者情報（開示されている場合）
 * - 回答・削除アクション
 */
...
export interface YoruqQuestion {
	id: string;
	text: string;
...
const emit = defineEmits<{
	(ev: 'answer', question: YoruqQuestion): void;
	(ev: 'delete', question: YoruqQuestion): void;
	(ev: 'report', question: YoruqQuestion): void;
	(ev: 'mute', question: YoruqQuestion): void;
}>();
</script>

<template>
<div class="yoruq-question-card">
	<!-- メッセージカードプレビュー -->
	<div class="card-preview">
		<YoruqMessageCard :question="question" />
	</div>
	...
	<div class="meta">
		<div class="sender-info">
			...
			<div v-else-if="question.status === 'answered'" class="sender answered-status">
				...
				<span>{{ i18n.ts._yoruq.statusAnswered }}</span>
			</div>
			<!-- 未回答かつ匿名の場合: 匿名表示 -->
			<div v-else class="sender anonymous">
				{{ i18n.ts._yoruq.anonymous }}
			</div>
		</div>
		<div class="timestamps">
			...
			<div v-if="question.status === 'answered' && question.answeredAt" class="time answered">
				<span class="label">{{ i18n.ts._yoruq?.answerDate ?? '回答' }}</span>
	...
	<div v-if="isModerator && question.senderUsername" class="moderator-info">
		<span class="disclosed-sender">
			{{ i18n.ts._yoruq.disclosedSender }}: @{{ question.senderUsername }}{{ question.senderHost ? `@${question.senderHost}` : '' }}
		</span>
	</div>
</div>
</template>
...
.yoruq-question-card {
	...
	.card-preview {
		...
		// YoruqMessageCardのダウンロードボタンを非表示
		:deep(.card-actions) {
			display: none;
		}
