...
import { encrypt as encryptE2E, isValidPublicKey } from '@/utility/yoruq-crypto.js';
...
import YoruqMessageCard from './YoruqMessageCard.vue';
...
const visibility = ref<Visibility>((localStorage.getItem('yoruq_answer_visibility') as Visibility) ?? 'public');
...
// メッセージカードコンポーネント参照
const messageCardRef = ref<InstanceType<typeof YoruqMessageCard>>();
...
async function postAnswer() {
	...
	try {
		...
		localStorage.setItem('yoruq_answer_visibility', visibility.value);
		...
		if (isE2EQuestion.value && canE2EAnswer.value && props.senderE2EPublicKey) {
			...
			await misskeyApi('yoruq/questions/answer', {
		...
		if (includeCard.value && messageCardRef.value) {
			...
			const { filePromise } = uploadFile(blob, {
				name: `yoruq-card-${props.question.id}.png`,
			});
		...
		console.error('[YoruqAnswerDialog] Failed to post answer:', err);
		// ユーザーにエラーを表示
		...
		os.alert({
			type: 'error',
			title: i18n.ts._yoruq?.answerFailed ?? '回答の送信に失敗しました',
			text: errorMessage,
		});
	} finally {
		posting.value = false;
...
>
	<template #header>
		...
		{{ i18n.ts._yoruq.answer }}
	</template>

	<div class="yoruq-answer-dialog">
		<!-- E2E暗号化質問の場合の注意 -->
		<MkInfo v-if="isE2EQuestion" class="e2e-notice">
			<i class="ti ti-lock"></i>
			{{ i18n.ts._yoruq?.e2eAnswerNote ?? 'これは暗号化された質問です。回答は質問者にのみDMで送信されます。' }}
		</MkInfo>

		<!-- 質問内容プレビュー -->
		<div class="question-preview">
			<div class="label">{{ i18n.ts._yoruq.questionText }}</div>
			<div v-if="isE2EQuestion" class="encrypted-text">
				<i class="ti ti-lock"></i>
				{{ i18n.ts._yoruq?.encryptedQuestion ?? '暗号化された質問' }}
			</div>
		...
		<MkTextarea
			v-model="answerText"
			:placeholder="i18n.ts._yoruq.answerPlaceholder ?? '回答を入力...'"
		...
		<div v-if="!isE2EQuestion" class="card-option">
			...
			<MkInfo v-if="isQuestionTextLong && !includeCard" class="long-question-hint">
				{{ i18n.ts._yoruq?.longQuestionHint ?? '質問文が長いため、CWでは省略されます。メッセージカードの添付がおすすめです。' }}
			</MkInfo>

			<MkSwitch v-model="includeCard">
				<template #label>
					<i class="ti ti-photo"></i>
					{{ i18n.ts._yoruq.includeMessageCard ?? 'メッセージカードを添付' }}
				</template>
			</MkSwitch>
		...
		<div class="actions">
			...
			<MkButton primary :disabled="posting || !answerText.trim()" @click="postAnswer">
				{{ posting ? i18n.ts.sending : i18n.ts._yoruq.answer }}
			</MkButton>
		</div>
	</div>
</MkModalWindow>
